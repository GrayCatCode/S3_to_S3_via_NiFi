...
DOWNLOAD_DIR="${DOWNLOAD_DIR:-/home/rocky/SQS_demo/Version_2/data/downloads}"
...
while true; do
  ...
  echo $MESSAGES_JSON > sqs_messages.json
  ...
  for (( i=0; i<MESSAGE_COUNT; i++ )); do
    ...
    then
      ...
      echo "Received invalid SQS message ..."

      aws sqs delete-message \
      --queue-url "$SQS_QUEUE_URL" \
      --receipt-handle "$RECEIPT_HANDLE" >/dev/null

      echo "Invalid message deleted."

    elif aws s3 cp "s3://$BUCKET/$KEY" "$DEST_PATH";
    then

      echo "✔ File download from S3 successful: $DEST_PATH"

      aws sqs delete-message \
        --queue-url "$SQS_QUEUE_URL" \
        --receipt-handle "$RECEIPT_HANDLE" >/dev/null

      echo "✔ Deleted the message from SQS for file: $DEST_PATH"
